...
b2 >> bass(dur=4,chop=3,sus=3, amp=0.3)
...
v2 >> loop("t2v3",P[0:16],dur=1,amp=var([2,0],4),mix=0.8,room=0.8,formant=[2])
...
v2 >> loop("t2v5",P[0:8],dur=PSum(1,1),amp=var([1],4),mix=0.9,room=0.9,formant=2)

v3 >> loop("t2v7",P[0:16],dur=PSum(9,4),amp=var([1,0],4),mix=0.9,room=0.9,formant=linvar([0,0]))
...
#intro
ps >> saw(P[0,2,1].palindrome(), dur=4, sus=3 oct=(5), chop=[6,8], amp=1.4) + var([0,-1,-2],4)
...
m2 >> piano(PWalk(4),dur=0.5,amp=1,pan=[-1,1],scale=Scale.minorPentatonic)
#m1.stop()
#m2.stop()
...
#el buen quin
m2 >> quin(var(chords), oct=[4,5], dur=PDur(5,8), amp=0.3, hpf=linvar([300, 1000]), shape=0.5) + var([0,2,-1])
...
v2 >> loop("t3v2",P[0:16],dur=PSum(8,4),amp=var([2,0],4),mix=0.7,room=0.7,formant=0)


v1.stop()

v1 >> loop("t3v1",P[0:16],amp=var([0,1.5],4),mix=0.9,room=0.9,formant=2)

# Instrumentis
# bajo del coro con shape
b1 >> bass(var([0,-1],[8,2]),dur=1,chop=var([8,16],5),amp=0.7, pan=[1,-1], shape=0.3)

#bips del tema anterior
bb >> play('{[bb] b}{b }{[bb]bb }',dur=1/2, rate=1, amp=PRand(1))

#ambi cosmico # SIRENAAAAAA
y1 >> ambi(linvar([0,5],16), dur=1/8, cut=0, chop=0, room=0.6, mix=0.8, pan=linvar([-1,1]), amp=linvar(P[1,1.5,1.5,1]/2))
...
d1 >>
play('o|*3|', sample=2, dur=0.25, amp=1)

m1 >> charm(PWalk(5),amp=0.7,chop=4,scale=Scale.minorPentatonic, dur=1/8, oct=(4,5), pan=linvar([-1,1],8))
